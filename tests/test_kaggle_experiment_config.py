...
def test_full_requirements_pin_python312_compatible_paddleocr_stack() -> None:
    ...
    assert "zxing-cpp>=2.2.0" in requirements
    assert "rapidfuzz>=3.9.0" in requirements
    assert "onnxruntime>=1.18.0" in requirements
...
def test_kaggle_experiment_logs_hardened_pipeline_knobs() -> None:
    ...
    assert "EXP_PIPELINE_DEFERRED_QR_TOP_K" in source
    assert "EXP_QR_NATIVE_MAX_VARIANTS" in source
    assert "EXP_QR_ENABLE_POINT_REGIONS" in source
    assert "EXP_INSTALL_ZBAR" in source
    assert "libzbar0" in source
    assert "EXP_TEMPORAL_QR_RECONSTRUCTION" in source
    assert "temporal_qr_reconstruction" in source
